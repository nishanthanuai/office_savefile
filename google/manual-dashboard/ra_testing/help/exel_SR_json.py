...
def generate_json_from_folder(folder_path, output_json_path="MCW_final.json"):
        ...
        if file_name.endswith(".xlsx"):
            ...
            if "MCW LHS" in file_name:
                ...
                if match:
                    side = match.group(2)
                    if side == "LHS":
                        road_name = f"Service Road LHS {1} (SRL{1})"
                        lhs_service_counter += 1  
                    else:
                        road_name = f"Service Road RHS {1} (SRR{1})"
                        rhs_service_counter += 1  
                else:
                ...
                if match:
                    side = match.group(3)
                    if side == "LHS":
                        road_name = f"Intersecting road LHS  {1} (IRL{1})" if match.group(1) == "T" else f"Intersecting road LHS{lhs_service} (CRL{lhs_service})"
...
# Example usage
folder_path = r"C:\\Users\\manav\\Desktop\\excel_folder_final_maker\\formatted_excel_script\\MCW"
