...
def generate_json_from_folder(folder_path, output_json_path, logger):
    ...
    logger.info(f"[CL_JSON] Processing folder: {folder_path}")
    ...
    for file_name in os.listdir(folder_path):
        ...
        try:
            ...
            logger.info(f"[CL_JSON] Starting processing file: {file_name}")
            ...
            if "MCW LHS" in file_name:
                ...
                if match:
                    ...
                    logger.warning(
                        f"[CL_JSON] Skipping unrecognized file: {file_name}")
                    continue
            ...
            logger.debug(
                f"[CL_JSON] Loaded dataframes shapes: {df.shape}, {df2.shape}")

            start_chainage_value = df.iloc[0, 1]
            logger.debug(
                f"[CL_JSON] Raw start chainage value: {start_chainage_value}")
            ...
            logger.debug(
                f"[CL_JSON] Normalized start chainage value: {start_chainage_value}")

            if start_chainage_value.isdigit() and start_chainage_value != "":
                from_value = int(start_chainage_value)
                to_value = (from_value // 50 + 1) * 50
                logger.debug(
                    f"[CL_JSON] Calculated range from {from_value} to {to_value}")
            else:
                logger.warning(
                    f"[CL_JSON] Invalid start chainage in {file_name}")
                continue

            logger.debug(f"[CL_JSON] Determined road name: {road_name}")
            ...
            for idx, row in df2.iterrows():
                ...
                logger.debug(
                    f"[CL_JSON] Row {idx}: type={asset_type}, side={side}, count={count}")
            ...
            logger.info(
                f"[CL_JSON] Added entry for {road_name} range {range_key}")

        except Exception:
            logger.exception(f"[CL_JSON] Failed processing file {file_name}")

    with open(output_json_path, "w") as json_file:
        json.dump(furniture_chainage_report, json_file, indent=4)

    logger.info(f"[CL_JSON] JSON created at {output_json_path}")
...
def run(survey_root: str, json_output: str, logger):

    logger.info("[CL_JSON] Stage started")
    logger.debug(f"[CL_JSON] survey_root={survey_root}")

    folder_path = os.path.join(survey_root, "Downloaded_Excels", "CL")

    json_dir = json_output
    os.makedirs(json_dir, exist_ok=True)

    output_json_path = os.path.join(json_dir, "CL_final.json")

    if not os.path.exists(folder_path):
        logger.warning("[CL_JSON] CL folder not found. Skipping.")
        return {"status": "skipped", "reason": "CL folder missing"}
    ...
    logger.info("[CL_JSON] Stage completed")
