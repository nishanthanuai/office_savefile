...
def generate_json_from_folder(folder_path, output_json_path, logger):
    ...
    IR_END = {}
    logger.info(f"[TL_JSON] Generating JSON from folder: {folder_path}")
    logger.debug(
        f"Initial counters lhs_service={lhs_service_counter}, rhs_service={rhs_service_counter}")
    ...
    for file_name in os.listdir(folder_path):
        ...
        if file_name.endswith(".xlsx"):
            ...
            logger.info(f"[TL_JSON] Processing file: {file_name}")
            ...
            if "MCW LHS" in file_name:
                ...
                if match:
                    ...
                    if side == "LHS":
                        ...
                        logger.debug(
                            f"Service road side=LHS; assigned {road_name}")

                    else:
                        road_name = f"Service Road RHS {rhs_service_counter} (SRR{rhs_service_counter})"
                        logger.debug(
                            f"Service road side=RHS; assigned {road_name}")
                ...
                if start_chainage_value.isdigit() and start_chainage_value != "":
                    ...
                    logger.warning(
                        f"Skipping file {file_name} as 'Start Chainage' is not valid.")
                    continue
                ...
                logger.debug(f"IR_END counts updated: {IR_END}")

                road_name = f"Intersection (Right below structure) (I{IR_END[str(to_value)]})"
                logger.debug(f"Assigned intersection road_name={road_name}")
                ...
                logger.info(
                    f"[TL_JSON] Added entry for {road_name} range {range_key}")
                continue

    logger.debug(
        f"Final report contains {len(furniture_chainage_report)} road sections")
    with open(output_json_path, "w") as json_file:
        json.dump(furniture_chainage_report, json_file, indent=4)

    logger.info(f"JSON file created successfully at: {output_json_path}")
...
def run(downloaded_excels_root, output_dir, logger=None):
    ...
    logger.info("Running TL_final module")

    tl_folder = os.path.join(downloaded_excels_root, "TL")

    if not os.path.exists(tl_folder):
        logger.warning(f"TL folder not found: {tl_folder}")
        return None

    os.makedirs(output_dir, exist_ok=True)

    output_json = os.path.join(output_dir, "TL_final.json")

    logger.info(f"Processing TL folder: {tl_folder}")

    result = generate_json_from_folder(
        tl_folder,
        output_json,
        logger
    )

    logger.info("TL module completed")
