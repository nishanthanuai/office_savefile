...
#     wb.save(wb_path)
#     print(f"Encroachment Signs sheet added to {wb_path}")
...
def apply_common_formatting(sheet, survey_data, road_data:Dict ):
...
# def create_detailed_report(data, survey_data, output_folder, road_id, output_json_path):
def create_detailed_report(data, survey_data, output_folder, road_id, output_json_path, road_data):
    ...
    filtered_data = [
        ...
        if (
            item.get("Asset type") in [
                "ADVERTISEMENT_ENCHROACHMENT_SIGNS", "NON_STANDARD_INFOMATORY_SIGNS"]
    ...
    logger.info(
        "Encroachment Signs sheet added successfully | road_id=%s | path=%s",
        road_id,
        output_file_path
    )
...
def run(output_json_path: str, output_folder: str, road_id: int, road_data: dict):
    """
    Master entry point for excel4.
    Creates Encroachment Signs sheet.
    """

    logger.info("Starting excel4 | road_id=%s", road_id)

    # Resolve road JSON file
    ...
    create_detailed_report(
        ...
        output_folder,
        road_id,
        output_json_path, road_data
    )

    logger.info("excel4 completed successfully | road_id=%s", road_id)
